...
public final class PatternView extends PasscodeView {
    ...
    @Override
    public void setCorrectPasscode(int[][] passcode) {
        setCorrectPattern(passcode);
    }
    ...
    public void setCorrectPattern(@NonNull int[][] patternArray) {
        // PatternPoint[] patternPoints = new PatternPoint[patternArray.length];
        int numOfPoints = patternArray.length;
        mCorrectPattern = new PatternPoint[numOfPoints];
        for (int i = 0; i < numOfPoints; i++) {
            mCorrectPattern[i] = new PatternPoint(patternArray[i]);
        }
        mPatternTyped.clear();
        invalidate();
    }

    public void setCorrectPattern(@NonNull ArrayList<PatternCell> patternList) {
        int numOfPoints = patternList.size();
        mCorrectPattern = new PatternPoint[numOfPoints];
        for (int i = 0; i < numOfPoints; i++) {
            PatternCell cell = patternList.get(i);
            mCorrectPattern[i] = new PatternPoint(cell.getPoint().x, cell.getPoint().y);
        }
        mPatternTyped.clear();
        invalidate();
    }
    ...
    public ArrayList<PatternCell> getPatternTyped() {
        return mPatternTyped;
    }

    @Override
    public int[][] getTypedPasscode() {
        int[][] pattern = new int[mPatternTyped.size()][2];

        for (int i = 0; i < mPatternTyped.size(); i++) {
            pattern[i][0] = mPatternTyped.get(i).getPoint().x;
            pattern[i][1] = mPatternTyped.get(i).getPoint().y;
        }
