...
public final class PinView extends PasscodeView implements InteractiveArrayList.ChangeListener {
    ...
    private void onKeyPressed(@Nullable String newDigit) {
        ...
        } else if ((mCorrectPin == null || mCorrectPin.length == 0) && !isRecording()) {
        ...
        } else if (getPinLength() > mPinTyped.size()) {
        ...
        if (getPinLength() == mPinTyped.size()) {   //Only check for the pin validity if typed pin has the length of correct pin.
            if (isRecording()) {
    ...
    public void setCorrectPasscode(int[][] passcode) {
        setCorrectPin(passcode);
    }

    @Override
    public int[][] getTypedPasscode() {

        return getCurrentTypedPin();
    }
    ...
    public int[][] getCurrentTypedPin() {
        int[][] arr = new int[1][mPinTyped.size()];
        for (int i = 0; i < mPinTyped.size(); i++) arr[0][i] = mPinTyped.get(i);
        return arr;
    }
    ...
    public void setCurrentTypedPin(int[] currentTypedPin) {
        ...
        } else if (currentTypedPin.length > getPinLength()) {
    ...
    public int getPinLength() {
        ...
        return mCorrectPin.length;
    }

    public void setPinLength(int length) {
        mBoxIndicator.setPinLength(length);
        requestLayout();
        invalidate();
    }
}
